...
async def test_private_store_stream_range_signed_expiry_and_path_guard(tmp_path: Path):
    ...
    signed = store.presign_download("assets/a/v1", 60)
    assert store.verify_download(signed)
    with pytest.raises(ValueError):
        await store.put_stream("../escape", chunks(b"x"), max_bytes=2)
...
def test_ssrf_rejects_private_and_non_https(monkeypatch):
    ...
    with pytest.raises(UnsafeRemoteURL):
        guard.validate("https://example.com/a.png")
    with pytest.raises(UnsafeRemoteURL):
        guard.validate("http://example.com/a.png")


def test_manifest_is_deterministic_and_generates_unicode_captions():
    spec = CourseBuildSpec.model_validate(
        {
            "request_id": "test-request-01",
                    ...
                    "lessons": [
                        {
                            ...
                            "script": {
                                "narration": "Hello world. नमस्ते दुनिया!",
                                "scenes": [
                                    {"id": "a", "type": "text", "on_screen_text": ["Hello"]}
                                ],
                            },
                        }
                    ],
    ...
    assets = {}
    first = RenderManifestCompiler().compile(spec, assets)
    second = RenderManifestCompiler().compile(spec, assets)
